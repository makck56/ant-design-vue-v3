  ...
  return defineComponent<MergedRangePickerProps<DateType>>({
    ...
    setup(props, { attrs, expose }) {
      ...
      const [mergedActivePickerIndex, setMergedActivePickerIndex] = useMergedState<0 | 1>(0, {
        value: toRef(props, 'activePickerIndex') as any,
      });
      ...
      const { width: containerWidth } = useElementSize(containerRef as any);
      watch([mergedOpen, containerWidth], () => {
        if (!mergedOpen.value && containerRef.value) {
          popupMinWidth.value = containerWidth.value;
        }
      });
      const { width: panelDivWidth } = useElementSize(panelDivRef as any);
      const { width: arrowWidth } = useElementSize(arrowRef as any);
      const { width: startInputDivWidth } = useElementSize(startInputDivRef as any);
      const { width: separatorWidth } = useElementSize(separatorRef as any);
      watch(
        [
